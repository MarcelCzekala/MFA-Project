...
enum class AppState : uint8_t {
  HOME, VERIFY_NFC, VERIFY_FINGER, VERIFY_HTTP, VERIFY_RESULT,
  ENROLL_FETCH_ID, ENROLL_NFC, ENROLL_FINGER, ENROLL_HTTP, ENROLL_RESULT,
...
static void readJoystickMenu() {
  ...
  if (now - menuNavigationDelayMs > 250) {
    if (v < MENU_LEFT) {
      ...
      if (currentMenuIndex < 2) { currentMenuIndex++; menuNavigationDelayMs = now; playFeedback("CLICK"); }
    }
  }
}
...
static void tickHome() {
  ...
  if (currentMenuIndex == 0) { appState = AppState::VERIFY_NFC; lcdTwo("Scan NFC card", "..."); }
  else if (currentMenuIndex == 1) { appState = AppState::ENROLL_FETCH_ID; lcdTwo("Fetching ID...", "Please wait"); }
  else if (currentMenuIndex == 2) { appState = AppState::CONFIRM_CLEAR_MEM; lcdTwo("Clear memory?", "Click to confirm"); }
}
...
static bool httpCheckCard(String uid) {
  ...
  String resp = http.getString();
  http.end();
  if (code == 200) { JsonDocument doc; deserializeJson(doc, resp); return doc["exists"] | false; }
  return false;
}
...
static void tickVerifyNfc() {
  ...
  if (httpCheckCard(uid)) { scannedNfcUid = uid; appState = AppState::VERIFY_FINGER; lcdTwo("Card OK", "Scan finger"); playFeedback("CLICK"); }
  else { lcdTwo("Unknown Card", "Access Denied"); playFeedback("ERROR"); appState = AppState::VERIFY_RESULT; resultUntilMs = millis() + RESULT_MS; }
}

static void tickVerifyFinger() {
  if (fingerprintStartTime == 0) { fingerprintStartTime = millis(); failedFingerAttempts = 0; }
  int id = readFingerMatch();
  if (id > 0) {
    scannedFingerId = id; appState = AppState::VERIFY_HTTP; lcdTwo("Sending...", "Please wait");
    fingerprintStartTime = 0; failedFingerAttempts = 0;
  } else {
    unsigned long now = millis();
    if (finger.getImage() == FINGERPRINT_OK) {
      failedFingerAttempts++;
      playFeedback("ERROR");
      if (failedFingerAttempts >= 2) {
        bool granted = false; String msg = "";
        httpVerifyMfa(scannedNfcUid, -1, granted, msg);
        lcdTwo("Access Denied", "Mismatch/Failed");
        appState = AppState::VERIFY_RESULT; resultUntilMs = millis() + RESULT_MS;
        fingerprintStartTime = 0; failedFingerAttempts = 0;
      } else { lcdTwo("Wrong Finger", "Try again..."); delay(1500); lcdTwo("Scan finger", "..."); }
    }
    if (now - fingerprintStartTime > 15000UL) {
      lcdTwo("Auth Failed", "Timeout"); playFeedback("ERROR");
      appState = AppState::VERIFY_RESULT; resultUntilMs = millis() + RESULT_MS;
      fingerprintStartTime = 0; failedFingerAttempts = 0;
    }
  }
...
static void tickVerifyHttp() {
  bool granted = false; String msg = "";
  if (httpVerifyMfa(scannedNfcUid, scannedFingerId, granted, msg)) {
    if (granted) { lcdTwo("Success", "Welcome!"); playFeedback("SUCCESS"); }
    else { lcdTwo("Failed", "Access Denied"); playFeedback("ERROR"); }
  } else { lcdTwo("Net Error", "Access Denied"); playFeedback("ERROR"); }
  appState = AppState::VERIFY_RESULT; resultUntilMs = millis() + RESULT_MS;
}
...
static void tickEnrollFetchId() {
  ...
  else { lcdTwo("Server Error", "Check Spring"); playFeedback("ERROR"); appState = AppState::ENROLL_RESULT; resultUntilMs = millis() + RESULT_MS; }
}
...
static void tickEnrollFinger() {
  if (millis() > enrollDeadlineMs) { lcdTwo("Enroll timeout", ""); playFeedback("ERROR"); appState = AppState::ENROLL_RESULT; resultUntilMs = millis() + RESULT_MS; return; }
  switch (enrollFingerPhase) {
    ...
    case EnrollFingerPhase::STORE: if (finger.createModel() == FINGERPRINT_OK && finger.storeModel(enrollNextId) == FINGERPRINT_OK) { appState = AppState::ENROLL_HTTP; lcdTwo("Saving user...", "Please wait"); } else { lcdTwo("Store failed", ""); playFeedback("ERROR"); appState = AppState::ENROLL_RESULT; resultUntilMs = millis() + RESULT_MS; } break;
  }
}

static void tickEnrollHttp() {
  if (httpRegisterUser(enrollNextId, enrollNfcUid, enrollNextId)) { lcdTwo("Enroll OK", "User saved"); playFeedback("SUCCESS"); }
  else { lcdTwo("Save failed", "Try again"); playFeedback("ERROR"); }
...
static void tickClearMemProcess() {
  if (finger.emptyDatabase() == FINGERPRINT_OK) { lcdTwo("Database Empty", "Wipe Complete"); playFeedback("SUCCESS"); }
  else { lcdTwo("Wipe Failed", "Driver Error"); playFeedback("ERROR"); }
  delay(2000); goHome();
}
...
void checkGlobalReset() {
  ...
  if (currentSwState == LOW) {
    if (swPressedStartMs == 0) swPressedStartMs = millis();
      ...
      if (appState != AppState::HOME && appState != AppState::CONFIRM_RESET) {
